...
// Motor A connections using bts7960 motor driver (Right Motor)
const int R_IS_A = A9;
const int L_IS_A = 37;
const int R_EN_A = 32;
const int L_EN_A = 23;
...
// Motor B connections using bts7960 motor driver (Left Motor)
const int R_IS_B = A6;
const int L_IS_B = 12;
const int R_EN_B = 16;
const int L_EN_B = 24;
...
void setup() {
  ...
  pinMode(R_IS_A, OUTPUT);
  pinMode(L_IS_A, OUTPUT);
  pinMode(R_IS_B, OUTPUT);
  pinMode(L_IS_B, OUTPUT);

  // Motors - Initial state
  digitalWrite(R_EN_B, HIGH);
  digitalWrite(L_EN_B, HIGH);
  digitalWrite(R_EN_A, HIGH);
  digitalWrite(L_EN_A, HIGH);
  digitalWrite(R_IS_A, HIGH);
  digitalWrite(L_IS_A, HIGH);  
  digitalWrite(R_IS_B, LOW);
  digitalWrite(L_IS_B, LOW);
